...
impl<'a> Visit<'a> for ScopeTree<'a> {
    ...
    fn visit_ts_type_name(&mut self, name: &TSTypeName<'a>) {
        if let TSTypeName::IdentifierReference(ident) = name {
            self.add_reference(ident.name.into_atom(), KindFlags::Type);
        } else {
        ...
        if let Some(declaration) = &decl.declaration {
            ...
            for specifier in &decl.specifiers {
                if let Some(name) = specifier.local.identifier_name() {
                    self.add_reference(name.into_atom(), KindFlags::All);
                }
            }
        }
    }
    ...
    fn visit_export_default_declaration(&mut self, decl: &ExportDefaultDeclaration<'a>) {
        if let ExportDefaultDeclarationKind::Identifier(ident) = &decl.declaration {
            self.add_reference(ident.name.into_atom(), KindFlags::All);
        } else {
        ...
        match declaration {
            ...
            Declaration::FunctionDeclaration(decl) => {
                if let Some(id) = decl.id.as_ref() {
                    self.add_binding(id.name.into_atom(), KindFlags::Value);
                }
            }
            ...
            Declaration::TSTypeAliasDeclaration(decl) => {
                self.add_binding(decl.id.name.into_atom(), KindFlags::Type);
            }
            Declaration::TSInterfaceDeclaration(decl) => {
                self.add_binding(decl.id.name.into_atom(), KindFlags::Type);
            }
            Declaration::TSEnumDeclaration(decl) => {
                self.add_binding(decl.id.name.into_atom(), KindFlags::All);
            }
            Declaration::TSModuleDeclaration(decl) => {
                if let TSModuleDeclarationName::Identifier(ident) = &decl.id {
                    self.add_binding(ident.name.into_atom(), KindFlags::All);
                }
            }
            ...
            Declaration::TSImportEqualsDeclaration(decl) => {
                self.add_binding(decl.id.name.into_atom(), KindFlags::Value);
            }
        }
        walk_declaration(self, declaration);
    }
